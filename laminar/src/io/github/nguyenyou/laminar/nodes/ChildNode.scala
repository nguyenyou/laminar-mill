...
trait ChildNode[+Ref <: dom.Node] extends ReactiveNode[Ref], Modifier[ReactiveElement[dom.Element]] {
  ...
  /** This is called as a notification, BEFORE changes to the real DOM or to the Scala DOM tree are applied.
    ...
    *
    * Note: This method is NOT automatically called inside [[setParent]] because [[setParent]] is called AFTER the real DOM was modified.
    * Therefore, IF you call [[setParent]] directly, you need to also call [[willSetParent]] before that, if you plan to implement that
    * method. However, if you only call [[setParent]] indirectly, via the methods defined in [[ParentNode]], those methods take care of
    * calling [[willSetParent]] for you.
    *
    * @param maybeNextParent
...
object ChildNode {
  ...
  /** Note: This walks up Laminar's element tree, not the real DOM tree. See [[io.github.nguyenyou.laminar.DomApi.isDescendantOf]] if you
    * want to check the real DOM tree.
    */
