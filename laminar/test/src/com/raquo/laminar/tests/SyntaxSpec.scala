...
class SyntaxSpec extends UnitSpec {
  ...
  it("seqToModifier, seqToNode implicit conversion works for both strings and nodes") {
    ...
    val mixed: immutable.Seq[Mod[HtmlElement]] = Vector("c", input())
    ...
    val actualNode = div(
      strings,
      stringsBuffer,
      stringsArray, // #TODO[IDE] IntelliJ 2023.3.4 highlights this as error
      stringsJsArray,
      nodes,
      nodesBuffer,
      nodesArray,
  ...
  it("Sinks and Sources and assorted type inference") {
    ...
    div(
      ...
      children <-- childrenSignal.map(laminar.Seq.from(_)),
