...
import io.github.nguyenyou.waypoint.fixtures.context.{PageBundle, SharedParams}
...
class ContextRouteBuilderSpec extends UnitSpec {
  ...
  def makeRouter = new Router[PageBundle](
    ...
    serializePage = page => write(page)(using PageBundle.rw),
    deserializePage = pageStr => read(pageStr)(using PageBundle.rw),
    owner = testOwner,
