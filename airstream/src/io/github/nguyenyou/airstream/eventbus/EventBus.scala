...
import io.github.nguyenyou.airstream.core.{EventStream, Named, Observer, Sink}
...
object EventBus {
  ...
  def emit(
    ...
    WriteBus.emit(values.map(toWriterTuple(_))*)
  }
  ...
  def emitTry(
    ...
    WriteBus.emitTry(values.map(toWriterTryTuple(_))*)
  }
