package io.github.nguyenyou.airstream.eventbus

import io.github.nguyenyou.airstream.core.{EventStream, InternalObserver, Observer, Transaction}
import io.github.nguyenyou.airstream.ownership.{Owner, Subscription}
import io.github.nguyenyou.airstream.util.hasDuplicateTupleKeys
...
class WriteBus[A](
  ...
  /** Hidden here because the public interface of WriteBus is all about writing rather than reading, but exposed in [[EventBus]]
    */
  private[eventbus] val stream: EventBusStream[A] = new EventBusStream(displayName)

  /** Note: this source will be removed when the `owner` you provide says so. To remove this source manually, call .kill() on the resulting
    * Subscription.
    */
  def addSource(sourceStream: EventStream[A])(using owner: Owner): Subscription = {
  ...
  def contracomposeWriter[B](operator: EventStream[B] => EventStream[A])(using owner: Owner): WriteBus[B] = {
  ...
  def contramapWriter[B](project: B => A)(using owner: Owner): WriteBus[B] = {
  ...
  def filterWriter(passes: A => Boolean)(using owner: Owner): WriteBus[A] = {
...
object WriteBus {
  ...
  /** Emit events into several WriteBus-es at once (in the same transaction) Example usage: emitTry(writeBus1 -> value1, writeBus2 ->
    * value2)
    */
  ...
  /** Emit events into several WriteBus-es at once (in the same transaction) Example usage: emitTry(writeBus1 -> Success(value1), writeBus2
    * -> Failure(error2))
    */
