...
/** Owner decides when to kill its subscriptions.
  ...
  *   - Owner can still be used after calling killPossessions, but the canonical use case is for the Owner to kill its possessions when the
  *     owner itself is discarded (e.g. a UI component is unmounted).
  *
...
trait Owner:
  ...
  /** This method will be called when this [[Owner]] has just started owning this resource. You can override it to add custom behaviour.
  ...
  private[ownership] def onKilledExternally(subscription: Subscription): Unit =
    ...
    if (index != -1) then subscriptions.splice(index, deleteCount = 1)
    else throw new Exception("Can not remove Subscription from Owner: subscription not found.")
