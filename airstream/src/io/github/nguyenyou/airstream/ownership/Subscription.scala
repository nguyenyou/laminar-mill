...
class Subscription(
  ...
  /** Make sure we only kill any given Subscription once. Just a sanity check against bad user logic, e.g. calling .kill() manually when
    * `owner` has already killed this subscription.
    */
