...
import io.github.nguyenyou.airstream.eventbus.WriteBus

// @TODO[API] I could make the constructor public but it's less confusing if you use the companion object methods

/** Represents a subscription that can be turned on and off repeatedly. For example, in Laminar the elements can be mounted and unmounted
  * repeatedly, and so their subscriptions are activated and deactivated respectively when those events happen.
  *
  * In contrast, the only thing you can do to a non-dynamic [[Subscription]] is `kill` it, and once that is done, it will remain dead
  * forever.
  *
  * Note that the dynamic subscription is NOT activated automatically upon creation.
  *
  * The subscription created by `activate` must not be killed externally, otherwise DynamicSubscription will throw when it tries to kill it
...
object DynamicSubscription:
  ...
  /** Use this when your activate() code requires cleanup on deactivation. Specify that cleanup code inside the resulting Subscription.
    *
    * Marked as "unsafe" because you must not kill() the subscription created by `activate`, it must be managed by this DynamicSubscription
    * only.
    *
    * @param activate
