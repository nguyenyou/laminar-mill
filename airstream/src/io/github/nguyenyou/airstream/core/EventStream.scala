...
import io.github.nguyenyou.airstream.combine.{CombineStreamN, MergeStream}
import io.github.nguyenyou.airstream.combine.generated._
import io.github.nguyenyou.airstream.core.Source.{EventSource, SignalSource}
...
import io.github.nguyenyou.airstream.debug.{DebuggableStream, Debugger, DebuggerStream}
import io.github.nguyenyou.airstream.distinct.DistinctStream
import io.github.nguyenyou.airstream.eventbus.EventBus
import io.github.nguyenyou.airstream.extensions._
import io.github.nguyenyou.airstream.javaflow.FlowPublisherStream
import io.github.nguyenyou.airstream.misc._
...
trait EventStream[+A] extends Observable[A], BaseObservable[EventStream, A], EventSource[A] {
  ...
  /** Filter stream events by a signal or var of boolean.
    *
    * `stream.filterWith(otherSignal, passes = _ == false)` is essentially like `stream.filter(_ => otherSignal.now() == false)` (but it
    * compiles)
    */
  ...
  /** Make a stream that emits this stream's values but waits for `after` stream to emit first in a given transaction. You can use this for
    * Signals too with `Signal.composeChanges` (see docs for more details)
    */
  ...
  /** Drop (skip) events from this stream as long as they pass the test (as soon as they stop passing, stop dropping) Note: errors are NOT
    * dropped.
    *
    * @param passes
    *   Note: MUST NOT THROW!
  ...
  /** Drop (skip) events from this stream as long as they do NOT pass the test (as soon as they start passing, stop dropping) Note: errors
    * are NOT dropped.
    *
    * @param passes
    *   Note: MUST NOT THROW!
  ...
  /** Take the first `numEvents` events from this stream, ignore the rest. Note: As long as events are being taken, ALL errors are also
    * taken
    *
    * @param resetOnStop
    *   Reset the count if the stream stops
  ...
  /** Returns a stream that emits events from this stream AND all off the `streams`, interleaved. Note: For other types of combination, see
    * `combineWith`, `withCurrentValueOf`, `sample` etc.
    */
  def mergeWith[B >: A](streams: EventStream[B]*): EventStream[B] = {
    val allStreams = this +: streams
    EventStream.merge(allStreams*)
  }
  ...
  /** See also various debug methods in [[io.github.nguyenyou.airstream.debug.DebuggableObservable]] */
...
object EventStream {
  ...
  /** Create a stream from a [[java.util.concurrent.Flow.Publisher]]
    *   - Use this to bring in events from other streaming libraries that can provide a `Flow.Publisher`, such as FS2 an Monix.
    */
  ...
  /** Easy helper for custom events. See [[CustomStreamSource]] for docs.
    *
    * Provide `start` and `stop` callbacks that will be called when the stream is started and stopped. E.g. create some resource on start,
    * clean it on stop.
    *
  ...
  def fromCustomSource[A](
    ...
    new CustomStreamSource[A]({ (fireValue, fireError, getStartIndex, getIsStarted) =>
      CustomSource
        .Config(
  ...
  def sequence[A](streams: Seq[EventStream[A]]): EventStream[Seq[A]] = {
    new CombineStreamN[A, Seq[A]](JsArray(streams*), _.asScalaJs.toSeq)
  }
  ...
  def merge[A](streams: EventStream[A]*): EventStream[A] = {
    new MergeStream[A](JsArray(streams*))
  }
  ...
  implicit def toTupleStream4[T1, T2, T3, T4](stream: EventStream[(T1, T2, T3, T4)]): TupleStream4[T1, T2, T3, T4] = new TupleStream4(
    stream
  )

  implicit def toTupleStream5[T1, T2, T3, T4, T5](stream: EventStream[(T1, T2, T3, T4, T5)]): TupleStream5[T1, T2, T3, T4, T5] =
    new TupleStream5(stream)

  implicit def toTupleStream6[T1, T2, T3, T4, T5, T6](stream: EventStream[(T1, T2, T3, T4, T5, T6)]): TupleStream6[T1, T2, T3, T4, T5, T6] =
    new TupleStream6(stream)

  implicit def toTupleStream7[T1, T2, T3, T4, T5, T6, T7](
    stream: EventStream[(T1, T2, T3, T4, T5, T6, T7)]
  ): TupleStream7[T1, T2, T3, T4, T5, T6, T7] = new TupleStream7(stream)

  implicit def toTupleStream8[T1, T2, T3, T4, T5, T6, T7, T8](
    stream: EventStream[(T1, T2, T3, T4, T5, T6, T7, T8)]
  ): TupleStream8[T1, T2, T3, T4, T5, T6, T7, T8] = new TupleStream8(stream)

  implicit def toTupleStream9[T1, T2, T3, T4, T5, T6, T7, T8, T9](
    stream: EventStream[(T1, T2, T3, T4, T5, T6, T7, T8, T9)]
  ): TupleStream9[T1, T2, T3, T4, T5, T6, T7, T8, T9] = new TupleStream9(stream)
}
