...
import io.github.nguyenyou.airstream.combine.CombineSignalN
import io.github.nguyenyou.airstream.combine.generated.{CombinableSignal, StaticSignalCombineOps}
import io.github.nguyenyou.airstream.core.Source.SignalSource
import io.github.nguyenyou.airstream.custom.{CustomSignalSource, CustomSource}
import io.github.nguyenyou.airstream.custom.CustomSource._
import io.github.nguyenyou.airstream.debug.{DebuggableSignal, Debugger, DebuggerSignal}
import io.github.nguyenyou.airstream.distinct.DistinctSignal
import io.github.nguyenyou.airstream.extensions._
import io.github.nguyenyou.airstream.misc.{MapSignal, ScanLeftSignal, StreamFromSignal}
import io.github.nguyenyou.airstream.ownership.Owner
import io.github.nguyenyou.airstream.split.{SplittableOneSignal, SplittableSignal}
...
/** Signal is an Observable,a current value. */
trait Signal[+A] extends Observable[A], BaseObservable[Signal, A], SignalSource[A] {
  ...
  /** A stream of all values in this signal, excluding the initial value.
    *
    * When re-starting this stream, it emits the signal's new current value if and only if something has caused the signal's value to be
    * updated or re-evaluated while the changes stream was stopped. This way the changes stream stays in sync with the signal even after
    * restarting.
    */
  ...
  /** See also debug methods in [[io.github.nguyenyou.airstream.debug.DebuggableObservable]]
    */
  ...
  /** Add a noop observer to this signal to ensure that it's started. This lets you access .now and .tryNow on the resulting StrictSignal.
    ...
    */
  def observe(using owner: Owner): OwnedSignal[A] =
    new ObservedSignal(this, Observer.empty, owner)

  override def toObservable: Signal[A] = this

  /** Here we need to ensure that Signal's default value has been evaluated at least once. It is important because if a Signal gets started
    * by means of its .changes stream acquiring an observer, nothing else would trigger this evaluation because initialValue is not directly
    * used by the .changes stream. However, when the events start coming in, we will need this initialValue because Signal needs to know
    * when its current value has changed.
    */
  ...
  /** Recalculate the signal's current value. Typically this asks the parent signal for its current value, and does the math from there,
    * according to the particular signal's logic (e.g. MapSignal would apply the `project` function).
    *
    * This method is used to calculate the signal's initial value, but also to recalculate, this signal's value, to re-sync it with the
    * parent, when this signal is restarted after being stopped. See https://github.com/raquo/Airstream/issues/43
    */
...
object Signal {
  ...
  /** The signal will start with `None`, even if the future is already resolved. Once the future resolves (or after a minimal async delay if
    * it's already resolved), the signal's value will be updated to the future's resolved value.
    */
  def fromFuture[A](
    future: Future[A]
  ...
  /** The signal will start with the provided `initial` value, even if the future is already resolved. Once the future resolves (or after a
    * minimal async delay if it's already resolved), the signal's value will be updated to the future's resolved value.
    */
  ...
  /** The signal will start with `None`, even if the promise is already resolved. Once the promise resolves (or after a minimal async delay
    * if it's already resolved), the signal's value will be updated to the promise's resolved value.
    */
  def fromJsPromise[A](promise: js.Promise[A]): Signal[Option[A]] = {
    new JsPromiseSignal(promise)
  }

  /** The signal will start with the provided `initial` value, even if the promise is already resolved. Once the promise resolves (or after
    * a minimal async delay if it's already resolved), the signal's value will be updated to the promise's resolved value.
    */
  ...
  /** A stream from js.Promise that kind-of sort-of behaves like a signal:
    ...
    *   - If you miss that event because this stream got stopped before the promise resolved, you will receive that event when you start
    *     this stream again
    *   - However, if this stream was not stopped, new subscribers will not receive the event. If you need such behaviour, use a proper
    *     signal instead.
    */
  ...
  /** Easy helper for custom signals. See [[CustomSignalSource]] for docs.
    *
    * Provide `start` and `stop` callbacks that will be called when the signal is started and stopped. E.g. create some resource on start,
    * clean it on stop.
    *
    * The arguments to `start` are functions. Call them to do things like update the signal's value, read its current value, or get some
    * info:
    *
    * `getStartIndex` returns `1` the first time the signal is started, and is incremented every time it is started again after being
    * stopped.
    *
    * `getIsStarted` is a function that you can call any time, including after some delay, to check if the signal is still started, e.g. if
  ...
  def sequence[A](
    ...
    new CombineSignalN[A, Seq[A]](JsArray(signals*), _.asScalaJs.toSeq)
  }
